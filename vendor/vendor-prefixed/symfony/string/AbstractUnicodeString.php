...
abstract class AbstractUnicodeString extends AbstractString
{
     ...
     *
     * @param string[]|\Kadence_Blocks_Transliterator[]|\Closure[] $rules See "*-Latin" rules from Transliterator::listIDs()
     */
    public function ascii(array $rules = []): self
    {
        ...
        while (\strlen($s) - 1 > $i = strspn($s, self::ASCII)) {
            ...
            if ($rule instanceof \Kadence_Blocks_Transliterator) {
                ...
                if ('nfd' === $rule = strtolower($rule)) {
                    ...
                    if (null === $transliterator = self::$transliterators[$rule] ?? self::$transliterators[$rule] = \Kadence_Blocks_Transliterator::create($rule)) {
                        if ('any-latin/bgn' === $rule) {
                            $rule = 'any-latin';
                            $transliterator = self::$transliterators[$rule] ?? self::$transliterators[$rule] = \Kadence_Blocks_Transliterator::create($rule);
                        }
    ...
    public function replaceMatches(string $fromRegexp, $to): parent
    {
        ...
        if (\is_array($to) || $to instanceof \Closure) {
            if (!\is_callable($to)) {
                throw new \Kadence_Blocks_TypeError(sprintf('Argument 2 passed to "%s::replaceMatches()" must be callable, array given.', static::class));
            }
    ...
    public function trimPrefix($prefix): parent
    {
        ...
        if ($prefix instanceof \Kadence_Blocks_Traversable) {
    ...
    public function trimSuffix($suffix): parent
    {
        ...
        if ($suffix instanceof \Kadence_Blocks_Traversable) {
