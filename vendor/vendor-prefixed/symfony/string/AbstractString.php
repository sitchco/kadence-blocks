...
abstract class AbstractString implements \Stringable, \JsonSerializable
{
    ...
    {
        if (!\is_array($suffix) && !$suffix instanceof \Kadence_Blocks_Traversable) {
            throw new \Kadence_Blocks_TypeError(sprintf('Method "%s()" must be overridden by class "%s" to deal with non-iterable values.', __FUNCTION__, static::class));
        }
    ...
    {
        if (!\is_array($string) && !$string instanceof \Kadence_Blocks_Traversable) {
            throw new \Kadence_Blocks_TypeError(sprintf('Method "%s()" must be overridden by class "%s" to deal with non-iterable values.', __FUNCTION__, static::class));
        }
    ...
    {
        if (!\is_array($needle) && !$needle instanceof \Kadence_Blocks_Traversable) {
            throw new \Kadence_Blocks_TypeError(sprintf('Method "%s()" must be overridden by class "%s" to deal with non-iterable values.', __FUNCTION__, static::class));
        }
    ...
    {
        if (null === $flags) {
            throw new \Kadence_Blocks_TypeError('Split behavior when $flags is null must be implemented by child classes.');
        }
    ...
    {
        if (!\is_array($prefix) && !$prefix instanceof \Kadence_Blocks_Traversable) {
            throw new \Kadence_Blocks_TypeError(sprintf('Method "%s()" must be overridden by class "%s" to deal with non-iterable values.', __FUNCTION__, static::class));
        }
