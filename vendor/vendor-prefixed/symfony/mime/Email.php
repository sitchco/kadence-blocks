...
class Email extends Message
{
    ...
    public function text($body, string $charset = 'utf-8')
    {
        if (null !== $body && !\is_string($body) && !\is_resource($body)) {
            throw new \Kadence_Blocks_TypeError(sprintf('The body must be a string, a resource or null (got "%s").', get_debug_type($body)));
        }
    ...
    public function attach($body, ?string $name = null, ?string $contentType = null)
    {
        if (!\is_string($body) && !\is_resource($body)) {
            throw new \Kadence_Blocks_TypeError(sprintf('The body must be a string or a resource (got "%s").', get_debug_type($body)));
        }
