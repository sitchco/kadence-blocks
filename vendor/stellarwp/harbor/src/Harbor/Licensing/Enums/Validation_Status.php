...
final class Validation_Status {
	...
	public static function message( string $value ): string {
		$messages = [
			self::EXPIRED                 => __( 'The entitlement has expired.', 'kadence-blocks' ),
			self::SUSPENDED               => __( 'The entitlement is suspended.', 'kadence-blocks' ),
			self::CANCELLED               => __( 'The entitlement is cancelled.', 'kadence-blocks' ),
			self::LICENSE_SUSPENDED       => __( 'The license is suspended.', 'kadence-blocks' ),
			self::LICENSE_BANNED          => __( 'The license is banned.', 'kadence-blocks' ),
			self::NO_ENTITLEMENT          => __( 'No entitlement exists for this product.', 'kadence-blocks' ),
			self::NOT_ACTIVATED           => __( 'The product is not activated on this domain.', 'kadence-blocks' ),
			self::OUT_OF_ACTIVATIONS      => __( 'All activation seats are in use.', 'kadence-blocks' ),
			self::INVALID_KEY             => __( 'The license key is not recognized.', 'kadence-blocks' ),
			self::ACTIVATION_REQUIRED     => __( 'The product requires activation.', 'kadence-blocks' ),
			self::TIER_SELECTION_REQUIRED => __( 'A tier selection is required for this product.', 'kadence-blocks' ),
		];

		return $messages[ $value ] ?? __( 'The license validation failed.', 'kadence-blocks' );
	}
